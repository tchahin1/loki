...
public class NotificationServiceImpl implements NotificationService {
    ...
    public Response pushUserWithToken(UserNotificationDto userNotificationDto) {
        ...
        if (alreadyInDb == null) {
            User user = userRepository.findByUsername(userNotificationDto.getUsername());
            if (user == null) return Response.status(400).entity("User does not exist!").build();
            String existingToken = this.gcmService.getDeviceTokenFromUsername(user.getUsername());
